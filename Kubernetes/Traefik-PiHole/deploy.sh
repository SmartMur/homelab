...
echo -e " \033[32;2m             https://github.com/dre/smartmur              \033[0m"
echo -e " \033[32;2m                                                           \033[0m"
...
if [ ! -d "`eval echo ${DESTINATION//>}`" ]; then
    ...
    mkdir Helm
    mkdir Manifest
    curl -L -o master.zip https://github.com/JamesTurland/Smartmur/archive/refs/heads/main.zip
    unzip master.zip -d ~/smartmur
    cp -r ~/smartmur/Smartmur-main/Kubernetes/Traefik-PiHole/* ~/
    rm master.zip
    rm -r ~/smartmur
...
kubectl apply -f ~/Helm/Traefik/Cert-Manager/Certificates/Production/smartmur-production.yaml
